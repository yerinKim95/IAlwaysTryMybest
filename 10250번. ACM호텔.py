...
이 정책에 따라 N 번째로 도착한 손님에게 배정될 방 번호를 계산하는 프로그램이다. 첫 번째 손님은 101 호, 두 번째 손님은 201 호 등과 같이 배정한다. 그림 1 의 경우를 예로 들면, H = 6이므로 10 번째 손님은 402 호에 배정해야 한다.
...
for i in range(T):
    ...
    if N % H == 0:
        floor.append( H )
        roomnum = N // H
        room.append(str(roomnum).zfill(2))
    else:
        floor.append( N % H )
        roomnum = N//H + 1
        room.append(str(roomnum).zfill(2))
for i in range(T):
    print('{0}{1}'.format(floor[i],room[i])) 
